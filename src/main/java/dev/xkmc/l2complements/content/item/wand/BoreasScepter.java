...
public class BoreasScepter extends Item {

	public static final int RANGE = 64, CHARGE = 100, SIZE_0 = 6, SIZE_1 = 16;

	public BoreasScepter(Properties properties) {
		super(properties);
	}
	...
	public static void tickServer(LivingEntity user, Level level, Vec3 center, int time) {
		WandEffectToClient.Type.BOREAS.send(user, center, time);
		...
		for (var e : list) {
			...
			if (time % 5 == 4)
				e.hurt(new DamageSource(DamageTypeGen.forKey(level, DamageTypeGen.FREEZE), user),
						LCConfig.COMMON.scepterOfBoreasDamage.get());
	...
	public static void tickClient(Level level, Vec3 center, int time) {
		...
		int n = Mth.clamp((int) radius, 10, 20);
		for (int i = 0; i < n; i++) {
			float tpi = (float) (Math.PI * 2);
			Vec3 v = new Vec3(0, 1, 0);
			v = v.xRot(tpi / 4).yRot(r.nextFloat() * tpi);
			Vec3 v0 = v.scale(SIZE_0 + r.nextFloat() * (radius - SIZE_0));
